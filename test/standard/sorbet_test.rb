...
class Standard::SorbetTest < Minitest::Test
  def test_that_it_has_a_version_number
    refute_nil ::Standard::Sorbet::VERSION
  end
  ...
  def test_configures_all_sorbet_cops
    expected = YAML.load_file(Pathname.new(Gem.loaded_specs["rubocop-sorbet"].full_gem_path).join("config/default.yml"))
  ...
  def test_does_not_require_rubocop_sorbet_and_change_the_default_rubocop_config
    ...
    assert defined?(RuboCop::Cop::Sorbet), "RuboCop::Cop::Sorbet not be defined (the cops aren't loaded and configuring them will blow up)"
    ...
    assert(
      defaults.to_h.none? { |k, v| k.start_with?("Sorbet") },
      "Sorbet cops should not be injected into RuboCop defaults (someone is requiring `rubocop-sorbet`)"
    )
  end

  def test_alphabetized_config
    skip "Wait to do this until we've simplified and finalized the YAML file"
